/*
Author: https://github.com/simco50
DOCUMENTATION
...
class StaticDelegate<RetVal(Args...), Args2...> : public IDelegate<RetVal, Args...>
{
	public:
		...
		explicit StaticDelegate(DelegateFunction function, Args2&&... payload)
...
{
	public:
		using DelegateFunction = typename DelegatesInteral::MemberFunction<IsConst, T, RetVal, Args..., Args2...>::Type;
...
class DelegateHandle
{
	public:
		...
		constexpr static unsigned int INVALID_ID = static_cast<unsigned int>(~0);
	private:
		unsigned int m_Id;
		inline static unsigned int CURRENT_ID;

		static unsigned int GetNewID()
		{
			const unsigned int output = CURRENT_ID++;
	...
	public:
		...
		void* Allocate(const size_t size)
		{
			if (m_Size != size)
			{
				...
				{
					pPtr = DelegatesInteral::Alloc(size);
					return pPtr;
				}
			}
			return (void*)Buffer;
...
class Delegate : public DelegateBase
{
	...
	public:
		...
		{
			Delegate handler;
			handler.template Bind<RawDelegate<false, T, RetVal(Args...), Args2...>>(pObj, pFunction, std::forward<Args2>(args)...);
			return handler;
		}
		...
		{
			Delegate handler;
			handler.template Bind<RawDelegate<true, T, RetVal(Args...), Args2...>>(pObj, pFunction, std::forward<Args2>(args)...);
			return handler;
		}
		...
		{
			Delegate handler;
			handler.template Bind<StaticDelegate<RetVal(Args...), Args2...>>(pFunction, std::forward<Args2>(args)...);
			return handler;
		}
		...
		{
			Delegate handler;
			handler.template Bind<SPDelegate<false, T, RetVal(Args...), Args2...>>(pObject, pFunction, std::forward<Args2>(args)...);
			return handler;
		}
		...
		{
			Delegate handler;
			handler.template Bind<SPDelegate<true, T, RetVal(Args...), Args2...>>(pObject, pFunction, std::forward<Args2>(args)...);
			return handler;
		}
		...
		NO_DISCARD static Delegate CreateLambda(TLambda&& lambda, Args2... args)
		{
			...
			handler.template Bind<LambdaDelegate<LambdaType, RetVal(Args...), Args2...>>(std::forward<LambdaType>(lambda), std::forward<Args2>(args)...);
			return handler;
		}
...
class MulticastDelegate : public DelegateBase
{
	...
	public:
		...
		//Bind a member function
		template<typename T, typename... Args2>
		DelegateHandle AddMember(T* pObject, NonConstMemberFunction<T, Args2...> pFunction, Args2&&... args)
		{
			return Add(DelegateT::CreateRaw(pObject, pFunction, std::forward<Args2>(args)...));
		}

		template<typename T, typename... Args2>
		DelegateHandle AddMember(T* pObject, ConstMemberFunction<T, Args2...> pFunction, Args2&&... args)
		{
			return Add(DelegateT::CreateRaw(pObject, pFunction, std::forward<Args2>(args)...));
		}
		...
		//Bind a member function with a shared_ptr object
		template<typename T, typename... Args2>
		DelegateHandle AddSharedMember(std::shared_ptr<T> pObject, NonConstMemberFunction<T, Args2...> pFunction, Args2&&... args)
		{
			return Add(DelegateT::CreateSP(pObject, pFunction, std::forward<Args2>(args)...));
		}

		template<typename T, typename... Args2>
		DelegateHandle AddSharedMember(std::shared_ptr<T> pObject, ConstMemberFunction<T, Args2...> pFunction, Args2&&... args)
		{
			return Add(DelegateT::CreateSP(pObject, pFunction, std::forward<Args2>(args)...));
		}
